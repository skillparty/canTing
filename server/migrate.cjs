...
async function runMigration() {
  ...
  try {
    ...
    if (loadSeeds) {
      ...
      const seedsSQL = fs.readFileSync(path.join(__dirname, '../database/seeds-simple.sql'), 'utf8');
...
async function checkConnection() {
  try {
    ...
    console.log('💡 Asegúrate de que PostgreSQL esté ejecutándose y que las credenciales sean correctas');
    process.exit(1);
  }
}
...
Uso:
  node migrate.cjs up [--seeds]    Ejecutar migración (con --seeds para datos de ejemplo)
  node migrate.cjs down            Rollback de migración
  node migrate.cjs test            Probar conexión a la base de datos
...
Ejemplos:
  node migrate.cjs up              # Solo crear esquema
  node migrate.cjs up --seeds      # Crear esquema y cargar datos de ejemplo
  node migrate.cjs down            # Eliminar todas las tablas
  node migrate.cjs test            # Probar conexión
    `);
}